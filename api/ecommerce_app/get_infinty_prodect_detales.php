...
$sql= " (SELECT product.*
,(SELECT photo_product.photo FROM photo_product WHERE photo_product.product_id=product.id AND photo_product.is_main_photo={$user_id}) AS main_photo

,(SELECT COUNT(like_product.id) FROM like_product WHERE like_product.product_id=product.id AND like_product.user_id={$user_id}) AS if_like
,(SELECT COUNT(save_product.id) FROM save_product WHERE save_product.product_id=product.id AND save_product.user_id={$user_id}) AS if_save
...
FROM `product` WHERE product.is_active=1 AND  id ={$product_id}
