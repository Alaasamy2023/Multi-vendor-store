...
,(SELECT COUNT(orders.id) FROM orders WHERE orders.product_id=product.id AND orders.user_id={$id} AND orders.active =1 ) AS if_orders

,(SELECT COUNT(like_product.id) FROM like_product WHERE like_product.product_id=product.id AND like_product.user_id={$id}) AS if_like
,(SELECT COUNT(save_product.id) FROM save_product WHERE save_product.product_id=product.id AND save_product.user_id={$id}) AS if_save
...
,(SELECT COUNT(review_product.id) FROM review_product WHERE review_product.product_id=product.id AND  review_product.active = 1) AS COUNT_rate_product
...
FROM `product` WHERE product.is_active=1   AND product.store_id = {$store_id}  
